...
import time
from Dynamic_Assignment import dynamic_assignment_clustering, save_dynamic_assignment_results
...
# Main function to run Dynamic Assignment and GA
def main():
    # Load the saved points
    points = load_points()
    num_uavs = 3

    # Step 1: Run Dynamic Assignment clustering
    start_time_clustering = time.time()
    clusters, centroids = dynamic_assignment_clustering(points, num_uavs)
    elapsed_time_clustering = time.time() - start_time_clustering

    # Save Dynamic Assignment results
    save_dynamic_assignment_results(clusters, centroids)

    # Print Dynamic Assignment results
    print("\n--- Dynamic Assignment Output ---")
    for i, cluster in enumerate(clusters):
        print(f"Cluster {i + 1} Waypoints:")
        print(np.array(cluster))
    print("\nCentroids:")
    print(centroids)
    print(f"Dynamic Assignment clustering completed in {elapsed_time_clustering:.2f} seconds.")

    # # Plot Dynamic Assignment clustering results
    # fig_clustering = plt.figure()
    # ax_clustering = fig_clustering.add_subplot(111, projection="3d")
    # ax_clustering.set_title("Dynamic Assignment Clustering")
    ...
    # Summary of timing
    print("\n--- Summary ---")
    print(f"Dynamic Assignment Time: {elapsed_time_clustering:.2f} seconds")
