from Path_Splitting import path_splitting_clustering  # Import Path Splitting function
...
def main():
    ...
    print("\n--- Path Splitting Input ---")
    print(f"Points:\n{points}")
    print(f"Number of UAVs: {num_uavs}")

    # Step 2: Run Path Splitting clustering
    start_time_clustering = time.time()  # Start timing Path Splitting
    clusters, rendezvous_point = path_splitting_clustering(points, num_uavs)
    elapsed_time_clustering = time.time() - start_time_clustering  # End timing Path Splitting
