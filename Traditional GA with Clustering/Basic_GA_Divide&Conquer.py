from Divide_And_Conquer import divide_and_conquer_clustering  # Import divide-and-conquer clustering function
...
def main():
    ...
    # Step 2: Run Divide-and-Conquer clustering
    start_time_clustering = time.time()  # Start timing Divide-and-Conquer
    clusters, centroids = divide_and_conquer_clustering(points, num_uavs)
    elapsed_time_clustering = time.time() - start_time_clustering  # End timing Divide-and-Conquer

    print("\n--- Divide and Conquer Output ---")
    for i, cluster in enumerate(clusters):
        print(f"Cluster {i + 1} Waypoints:\n{np.array(cluster)}")
    print(f"\nCentroids:\n{centroids}")
    print(f"Divide-and-Conquer clustering completed in {elapsed_time_clustering:.2f} seconds.")
    ...
    for i, cluster_points in enumerate(clusters_with_start):
        ...
        num_iterations = 50000  # Define the number of iterations for GA
    ...
    print(f"\n--- Timing Summary ---")
    print(f"Divide and Conquer Time: {elapsed_time_clustering:.2f} seconds")
