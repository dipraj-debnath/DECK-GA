from Nearest_Neighbor import nearest_neighbor_clustering  # Import Nearest Neighbor clustering function
...
def main():
    ...
    start_time_clustering = time.time()  # Start timing Nearest Neighbor
    clusters, centroids = nearest_neighbor_clustering(points, num_uavs, np.array(start_points))
    elapsed_time_clustering = time.time() - start_time_clustering  # End timing Nearest Neighbor

    print("\n--- Nearest Neighbor Output ---")
    for i, cluster in enumerate(clusters):
        print(f"Cluster {i + 1} Waypoints:\n{np.array(cluster)}")
    print(f"\nCentroids:\n{centroids}")
    print(f"Nearest Neighbor clustering completed in {elapsed_time_clustering:.2f} seconds.")
    ...
    print(f"\n--- Timing Summary ---")
    print(f"Clustering Time: {elapsed_time_clustering:.2f} seconds")
