from Load_Balancing import load_balancing_clustering  # Import Load Balancing function
...
def main():
    ...
    # Step 2: Run Load Balancing clustering
    start_time_clustering = time.time()  # Start timing Load Balancing
    clusters_dict, centroids = load_balancing_clustering(points, num_uavs)
    elapsed_time_clustering = time.time() - start_time_clustering  # End timing Load Balancing

    clusters = [np.array(clusters_dict[uav]) for uav in range(num_uavs)]
    ...
    print(f"\n--- Timing Summary ---")
    print(f"Load Balancing Clustering Time: {elapsed_time_clustering:.2f} seconds")
