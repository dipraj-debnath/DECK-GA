def ga_3d_pathplanning(xyz):
    ...
    for i in range(1, len(rearranged_route)):
        optimized_distance += np.linalg.norm(rearranged_route[i] - rearranged_route[i-1])
    optimized_distance += np.linalg.norm(rearranged_route[0] - rearranged_route[-1])  # Closing the loop
    ...
    return rearranged_route, optimized_distance
