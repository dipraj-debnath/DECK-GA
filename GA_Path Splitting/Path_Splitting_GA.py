...
import time
from Path_Splitting import path_splitting_clustering, save_path_splitting_results
...
# Main function to run Path Splitting and GA
def main():
    # Load the saved points
    points = load_points()
    num_uavs = 3

    # Step 1: Run Path Splitting clustering
    start_time_clustering = time.time()
    clusters, rendezvous_point = path_splitting_clustering(points, num_uavs)
    centroids = np.array([rendezvous_point] * num_uavs)  # Create centroids as duplicates of rendezvous_point
    elapsed_time_clustering = time.time() - start_time_clustering

    # Save Path Splitting results
    save_path_splitting_results(clusters, rendezvous_point)

    # Print Path Splitting results
    print("\n--- Path Splitting Output ---")
    for i, cluster in enumerate(clusters):
        print(f"Cluster {i + 1} Waypoints:")
        print(np.array(cluster))
    print("\nCentroids:")
    print(centroids)
    print(f"Path Splitting clustering completed in {elapsed_time_clustering:.2f} seconds.")

    # # Plot Path Splitting clustering results
    # fig_clustering = plt.figure()
    # ax_clustering = fig_clustering.add_subplot(111, projection="3d")
    # ax_clustering.set_title("Path Splitting Clustering")
    ...
    # Summary of timing
    print("\n--- Summary ---")
    print(f"Path Splitting Time: {elapsed_time_clustering:.2f} seconds")
