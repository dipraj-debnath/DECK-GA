...
import time
from kmeans_Updated_2 import manual_kmeans_clustering, save_kmeans_results
#from kmeans_Updated_3 import  enhanced_kmeans_clustering as manual_kmeans_clustering, save_kmeans_results

from GA_path_planning import ga_3d_pathplanning

def calculate_path_distance(path):
    """Calculate the total path distance for a given route."""
    distance = 0
...
def main():
    ...
    # Calculate and print input distances for each UAV
    print("\n--- Input Distances ---")
    for i, cluster in enumerate(clusters):
        cluster_with_start = np.vstack([[0, 0, 0], np.array(cluster)])
        input_distance = calculate_path_distance(cluster_with_start)
        print(f"UAV {i+1} Input Distance: {input_distance:.2f}")
    ...
    # Calculate and print optimized path distances
    print("\n--- Optimized Distances ---")
    for i, path in enumerate(optimized_paths):
        optimized_distance = calculate_path_distance(path)
        print(f"UAV {i+1} Optimized Distance: {optimized_distance:.2f}")
